        ...
        for i,df_dict in enumerate(data_dict):
            ...
            for measure in measures:
                ...
                for halo,cutout in enumerate(cutouts):
                    ...
                    #lensing is done here
                    cluster_redshift = catalog.data["redshift"].values[0]
                    # add lensing residual here...
                    zero_patch = np.zeros((nx,nx))
                    cmb = add_cmb(zero_patch, 0.5, L, Cl)

                    lensed_cmb = gp.getkappa_imports(cmb,[nx,nx,dx,dx], cluster_mass, cluster_redshift)
                    residual = lensed_cmb - cmb
                    
                    cutout = cutout+residual
                    if halo==1:
...
pickle.dump(result,gzip.open('mf_results_fs/%s_halos_%s_cmb_0.25_lensing_noise_%s.pkl.gz'%(total_no_samples,cmb_power,noise_level),'w'))
