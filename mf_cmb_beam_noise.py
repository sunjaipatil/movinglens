...
measures = ['v_th','v_ph']
cmb_dict = {"pixres": dx, "L" : [L], "Cl": [Cl],"B2l" : [B2l], "seed": list(2*halo_list+1)  }
beam_dict = {"pixres": dx, "L" : [L], "B2l" : [B2l]}

noise_dict = {"pixres": dx, "L" : [L], "Nl" : [Nl], "seed": list(2*halo_list+1)}
result['beam_dict'] = beam_dict
result['noise_dict'] = noise_dict
# effects to be added
...
result['func_list']  = str(func_list)
result['tmp_func_list']  = str(tmp_func_list)
result['func_kwargs_list'] = str(func_kwargs_list)
result['tmp_func_kwargs_list'] = str(tmp_func_kwargs_list)
...
for z in zbins:
    ...
    for mm in mbins:
        ...
        if len(dfzm['M_200c'])<total_no_samples:
            continue
...
result['Time taken'] = total_time
pickle.dump(result,gzip.open('mf_results_fs/%s_halos_%s_cmb_%s_noise_%s_beam_%s_pixres.pkl.gz'%(total_no_samples,cmb_power,noise_level,beam_size, reso),'w'))
