...
for z in zbins:
    ...
    for mm in mbins:
        ...
        for i,df_dict in enumerate(data_dict):
            ratio = abs(df_dict['v_th'])/abs(df_dict['v_ph'])
            if not (ratio>=0.9) & (ratio<=1.1):#abs(df_dict['v_ph'])>=10*abs(df_dict['v_th']):
...
pickle.dump(result,gzip.open('mf_results_websky_lite_veq.pkl.gz','w'))
